      ...
      <div className=' items-center mb-4 w-full'>
        <div className='comment'>
          <div className='flex h-7'>
            <div className='w-7 h-full rounded-full bg-blue-600 items-center mr-3'>
              {datas.img ? (
                ...
                <div className='text-center text-gray-50'>{datas.email[0]?.toUpperCase()}</div>
              )}
            </div>
            {datas.isAdmin ? (
          ...
          <div className='mt-4 flex'>
            <button className='text-blue-500 hover:underline' onClick={() => setIsVisible(!isVisible)}>
              Trả lời
            </button>
            <div className='ml-2 w-6 h-6 rounded-full bg-blue-600 mr-3 text-center text-gray-50'>?</div>
          </div>
        </div>
        {isVisible && (
